...
std::map<std::string, int> room_data_map = read_string_to_int_map("single.txt");
...
int main()
{
    ...
    InetAddress serverAddr("127.0.0.1", port_S);       // Server will bind to port
    ...
    cout << "The Server S is up and running using UDP on port " << getLocalPort(serverSocket.getFd()) << "." << endl;
    Socket_Packet_t packet;
    Server_Send_Room_Status_t send_room_status;
    packet.header.type = SERVERS_SEND_ROOM_STATUS;
