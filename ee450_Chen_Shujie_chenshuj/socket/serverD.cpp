...
#include "Util.h"
using namespace std;
std::map<std::string, int> room_data_map = read_string_to_int_map("double.txt");
...
int main()
{
    ...
    InetAddress serverAddr("127.0.0.1", port_D);       // Server will bind to port
    ...
    cout << "The Server D is up and running using UDP on port " << getLocalPort(serverSocket.getFd()) << "." << endl;
    Socket_Packet_t packet;
    Server_Send_Room_Status_t send_room_status;
    packet.header.type = SERVERD_SEND_ROOM_STATUS;
